...
createBill = (payload)=>{
  return new Promise((resolve, reject)=>{
    mysql_util.getConnection().then((resp)=>{
      ...
      conn.query(sql_constants.SQL_SP_BILLS_CREATE_BILL, 
        [idStore, customerId, customerName, customerPhone, customerAddress], (err, result)=>{
          conn.end();
          if(err){
            reject({
              result : constants.ERROR,
              message : "Error while executing " + constants.SQL_SP_BILLS_CREATE_BILL,
              err
            });
          } else{
...
addProductToBill = (idBill, idProduct, units, description, off) => {
  return new Promise((resolve, reject)=> {
    mysql_util.getConnection().then((resp)=>{
      let conn = resp;
      conn.query(sql_constants.SQL_SP_BILLS_ADD_PRODUCT_TO_BILL, 
        [idBill, idProduct, units, description, off], (err, result) => {
          conn.end();
          if(err){
            reject({
              result : constants.ERROR,
              message : "Error while executing " + sql_constants.SQL_SP_BILLS_ADD_PRODUCT_TO_BILL,
              err
            });
          } else{
            if(result[0][0] && result[0][0].ERROR == 0){
...
delProductFromBill = (idBill, idBillDetail) => {
  return new Promise((resolve, reject)=> {
    mysql_util.getConnection().then((resp)=>{
      let conn = resp;
      conn.query(sql_constants.SQL_SP_BILLS_DEL_PRODUCT_FROM_BILL,
        [idBill, idBillDetail], (err, result) => {
          conn.end();
          if(err){
            reject({
              result : constants.ERROR,
              message : "Error while executing " + sql_constants.SQL_SP_BILLS_DEL_PRODUCT_FROM_BILL,
              err
            });
          } else{
            if(result[0][0] && result[0][0].ERROR == 0){
...
updateProductFromBill = (idBill, idBillDetail, units, description, off) => {
  return new Promise((resolve, reject)=> {
    mysql_util.getConnection().then((resp)=>{
      let conn = resp;
      conn.query(sql_constants.SQL_SP_BILLS_UPDATE_PRODUCT_FROM_BILL,
        [idBill, idBillDetail, units, description, off], (err, result) => {
          conn.end();
          if(err){
            reject({
              result : constants.ERROR,
              message : "Error while executing " + sql_constants.SQL_SP_BILLS_UPDATE_PRODUCT_FROM_BILL,
              err
            });
          } else{
            if(result[0][0] && result[0][0].ERROR == 0){
    ...
    mysql_util.getConnection().then((resp)=>{
      ...
      conn.query(sql_constants.SQL_SP_BILLS_CHANGE_STATUS, [idBill, idStatus], (err, result) => {
        ...
        if(err){
          reject({
            result : constants.ERROR,
            message : "Error while executing " + sql_constants.SQL_SP_BILLS_CHANGE_STATUS
          });
        } else{
          if(result[0][0] && result[0][0].ERROR == 0){
            resolve({
...
searchBill = (idStore, query)=>{
  return new Promise((resolve, reject) => {
    mysql_util.getConnection().then((resp)=>{
      let conn = resp;
      conn.query(sql_constants.SQL_SP_BILLS_FIND_BILL, [idStore, query], (err, result) => {
        conn.end();
        if(err){
          reject({
            result : constants.ERROR,
            message : "Error while executing " + sql_constants.SQL_SP_BILLS_FIND_BILL
          });
        } else{
          resolve({
            result : constants.RESULT_OK,
...
getBill = (idStore, idBill) => {
  return new Promise((resolve, reject) => {
    mysql_util.getConnection().then((resp) => {
      let conn = resp;
      conn.query(sql_constants.SQL_SP_BILLS_GET_BILL, [idStore, idBill], (err, result) => {
        conn.end();
        if(err){
          reject({
            result : constants.ERROR,
            message : "Error while executing " + sql_constants.SQL_SP_BILLS_GET_BILL
          });
        } else{
    ...
    mysql_util.getConnection().then((resp) => {
      ...
      conn.query(sql_constants.SQL_SP_BILLS_GET_BY_STORE, [idStore, page, query, starts, ends], (err, result) => {
        ...
        if(err){
          reject({
            result : constants.ERROR,
            message : "Error while executing " + sql_constants.SQL_SP_BILLS_GET_BY_STORE
          });
        } else{
          resolve({
            result : constants.RESULT_OK,
...
getBillDetail = (idStore, idBill) => {
  return new Promise((resolve, reject) => {
    mysql_util.getConnection().then((resp) => {
      let conn = resp;
      conn.query(sql_constants.SQL_SP_BILLS_GET_BILL_DETAIL, [idStore, idBill], (err, result) => {
        conn.end();
        if(err){
          reject({
            result : constants.ERROR,
            message : "Error while executing " + sql_constants.SQL_SP_BILLS_GET_BILL_DETAIL
          });
        } else{
          resolve({
            result : constants.RESULT_OK,

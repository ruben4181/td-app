...
const pos_services = require(process.env.DIR_PATH+"/database/pos/pos");
...
addProductToBill = (idBill, idProduct, units, description, off) => {
  if(idBill && idProduct && units && typeof off == "number"){
    return pos_services.addProductToBill(idBill, idProduct, units, description, off);
  } else{
    return new Promise((resolve, reject) =>{
      resolve({
        result : constants.ERROR,
...
updateProductFromBill = (idBill, idBillDetail, units, description, off) => {
  if(idBill && idBillDetail && typeof off==="number"){
    return pos_services.updateProductFromBill(idBill, idBillDetail, units, description, off);
  } else{
    return new Promise((resolve, reject) => {
      resolve({
        result : constants.ERROR,

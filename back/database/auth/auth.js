...
authUser = (username, password) =>{
  return new Promise((resolve, reject)=>{
    mysql_util.getConnection().then((resp)=>{
      let conn = resp;
      conn.query(sql_constants.SQL_AUTH_USER, [username, password], (err, result)=>{
        conn.end();
        if(err){
          reject({
...
createUser = (payload) => {
  return new Promise((resolve, reject)=>{
    mysql_util.getConnection().then((resp)=>{
      ...
      conn.query(sql_constants.SQL_SP_INSERT, [userName, email, password, mongoId], (err, result)=>{
        conn.end();
        if(err){
          reject({
            result : constants.ERROR,
...
updateUser = (idUser, payload)=>{
  return new Promise((resolve, reject)=>{
    mysql_util.getConnection().then((resp)=>{
      ...
      let email = payload.email;
      conn.query(sql_constants.SQL_SP_UPDATE, [idUser, oldUsername, newUsername, oldPassword, newPassword, email], (err, result) => {
        conn.end();
        if(err){
          resolve({
            result : constants.ERROR,
...
getUserRoles = (idUser, idStore) =>{
  return new Promise((resolve, reject)=>{
    mysql_util.getConnection().then((resp)=>{
      let conn = resp;
      conn.query(sql_constants.SQL_SP_GET_ROLES, [idUser, idStore], (err, result)=>{
        conn.end();
        if(err){
          resolve({
            result : constants.ERROR,
